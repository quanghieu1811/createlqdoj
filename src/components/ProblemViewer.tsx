...
export const ProblemViewer: React.FC<ProblemViewerProps> = ({ problem, onRefine, refining }) => {
  const [activeTab, setActiveTab] = useState<'statement' | 'ioi' | 'generator' | 'solutions' | 'checker' | 'analysis'>('statement');
  const [activeSolutionIdx, setActiveSolutionIdx] = useState<number>(0);
  const [activeIoiLang, setActiveIoiLang] = useState<'cpp' | 'py'>('cpp');
  ...
  const handleDownloadZip = async () => {
    ...
    // 2. IOI Files (if present)
    if (problem.ioi) {
      const ioiFolder = zip.folder("ioi_grader");
      if (ioiFolder) {
        ioiFolder.file("header.h", problem.ioi.headerH);
        ioiFolder.file("handler.cpp", problem.ioi.handlerCpp);
        ioiFolder.file("handler.py", problem.ioi.handlerPy);
        ioiFolder.file("stub.cpp", problem.ioi.contestantStubCpp);
        ioiFolder.file("stub.py", problem.ioi.contestantStubPy);
        ioiFolder.file("signatures.txt", `C++ Signature:\n${problem.ioi.functionSignatureCpp}\n\nPython Signature:\n${problem.ioi.functionSignaturePy}\n`);
      }
      // Also put stubs in root for convenience
      zip.file("stub.cpp", problem.ioi.contestantStubCpp);
      zip.file("stub.py", problem.ioi.contestantStubPy);
      zip.file("header.h", problem.ioi.headerH);
    }

    // 3. Test Generator C++
    zip.file("generator.cpp", problem.testGenerator);

    // 3.5. Generator Script
...
${problem.ioi ? `2. ioi_grader/: Thư mục chứa header.h, handler.cpp, handler.py và stubs cho thí sinh.\n` : ""}
3. generator.cpp: Mã nguồn C++ sinh dữ liệu test ngẫu nhiên (đầu vào qua stdout, đáp án qua stderr).
4. script.txt: Script sinh test gồm đúng ${scriptLines} dòng tham số tương ứng với ${scriptLines} testcases mạnh.
5. solution_*: Mã nguồn giải thuật C++ mẫu cho từng subtask tương ứng.
6. checker.cpp: Trình chấm đặc biệt (Special Judge) nếu có.
7. interactive.cpp: Trình tương tác (Interactive Manager) nếu có.
8. analysis.txt: Phân tích thuật toán chi tiết.
    ...
    <div className="space-y-6">
        ...
        <div className="relative z-10 flex flex-col lg:flex-row items-start lg:items-center justify-between gap-6">
          <div className="space-y-2">
            <div className="flex flex-wrap items-center gap-2">
              ...
              {problem.ioi && (
                <span className="px-2.5 py-0.5 bg-purple-500/20 text-purple-300 border border-purple-500/30 text-xs font-bold rounded-full flex items-center gap-1">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        <div className="lg:col-span-3 space-y-2">
          ...
          {/* IOI Tab (if problem is IOI Mode) */}
          {problem.ioi && (
            <button
              type="button"
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <div className="text-xs font-bold flex items-center gap-1 text-purple-950">
                  <span>IOI Mode (Nộp hàm)</span>
                  <span className="w-1.5 h-1.5 rounded-full bg-purple-500"></span>
                </div>
                <div className="text-[10px] text-slate-400 font-normal truncate">Header, Handlers & Khung mẫu</div>
              </div>
              <ChevronRight className="w-4 h-4 text-slate-300" />
            </button>
          )}
            ...
            <div className="space-y-6">
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between border-b border-slate-100 pb-3 gap-3">
                <div>
                  <h3 className="text-base font-extrabold text-purple-950 flex items-center gap-2">
                    ...
                    IOI Signature & Bộ điều phối (Grader / Handler)
                  </h3>
                  <p className="text-xs text-slate-500 mt-0.5">
                    Thí sinh cài đặt hàm; hệ thống judge liên kết tự động với Handler
                  </p>
                </div>
              ...
              <div className="p-4 bg-purple-50/60 border border-purple-200 rounded-2xl space-y-2">
                ...
                <div className="font-mono text-xs bg-white p-3 rounded-xl border border-purple-100 text-purple-950 shadow-2xs">
                  {activeIoiLang === 'cpp' ? problem.ioi.functionSignatureCpp : problem.ioi.functionSignaturePy}
                </div>
                <p className="text-[11px] text-purple-800 leading-relaxed">
                  {activeIoiLang === 'cpp' 
                    ? "Thí sinh chỉ nộp thân hàm trên C++, include file header.h đi kèm. Judge sẽ biên dịch chung cùng handler.cpp."
                    : "Thí sinh nộp file bài làm (được lưu dưới tên _submission.py). Handler sẽ import hàm giải bài trực tiếp."}
                </p>
              </div>
              ...
              {activeIoiLang === 'cpp' && (
                <div className="space-y-6">
                  ...
                  <div className="space-y-2">
                    ...
                    <p className="text-xs text-slate-500">Chương trình đọc input từ luồng chuẩn, gọi hàm của thí sinh và in kết quả ra luồng chuẩn.</p>
              ...
              {activeIoiLang === 'py' && (
                <div className="space-y-6">
                  <div className="space-y-2">
                    ...
                    <p className="text-xs text-slate-500">Import hàm từ file bài nộp <code>_submission.py</code>, đọc dữ liệu và in đáp án.</p>
